...
DemoApplication.main()
  └── AppRunner.run()
        ...
        │     ├── imageBuffer (ConcurrentHashMap)          │ 图片缓存
...
```
ConcurrentHashMap<String, byte[]> imageBuffer      // key=userId, value=图片字节
ConcurrentHashMap<String, Long>   imageBufferTime  // key=userId, value=缓存时间戳
IMAGE_BUFFER_TTL = 5 * 60 * 1000                   // 5分钟有效期
```

```
场景1: 用户先发图再发文字（5分钟内）
  图片消息 → downloadImage → 缓存 imageBuffer[userId] + 记录时间戳
             → 回复 "已收到图片，请告诉我你想了解什么？"
  文字消息 → 检查缓存有图且未过期 → 取出 → 一起送去识图
             → 回复描述
...
用户: "这是什么" → 缓存取出 + 文字
  ↓
handleImageMessage() / handleMessage()
  ├── client.downloadImageFromMessageItem(imageItem)
  │     → CDN 下载 → byte[]
  ├── ImageService.recognizeImage(bytes, text)
  │     ├── 写临时文件
  │     ├── MultiModalConversation.call()    // qwen-vl-max
  │     │     ├── model: qwen-vl-max
  │     │     ├── content: [{"image":临时文件路径}, {"text":"这是什么"}]
  │     │     └── return 文字描述
  │     ├── 删临时文件
  │     └── return "图片里有一只鸡..."
  └── sendReply → client.sendText()
...
4. **图片缓冲机制**：微信图片和文字分开发，用 `imageBuffer[userId]` 临时缓存图片（5分钟过期），文字消息到达时取出一起识图
