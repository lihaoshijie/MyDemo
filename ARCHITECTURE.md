...
WeChatBotService.handleMessage(msg)
  ...
  ├── downloadFileFromMessageItem() → byte[]
  └── extractTextContent() 提取文本
        ├── .txt/.md/.json → UTF-8/GBK 直接读取
        ├── .docx → Apache POI (XWPFWordExtractor)
        ├── .xlsx → Apache POI (XSSFWorkbook)
        ├── .pdf  → Apache PDFBox
        └── 二进制 → OSSUtils.upload() → URL 传给 LLM
```

### 提取策略

```
isBinarySignature(bytes) → 非 Office 时检测 PK/PDF/PNG/JPEG 签名
Office 文件 (.docx/.xlsx/.pdf) → 专用库提取
文本文件 → UTF-8 → 可读字符>50%? → 通过
         → GBK   → 可读字符>50%? → 通过
         → 失败 → OSS 上传 → URL 传给 LLM
长度限制: 文本5万字, Office/PDF 5千字
```

### 总结文件生成

用户说"总结/导出" → 生成 `原文件名_总结.txt` → sendFile() 发回

### 依赖库

| 库 | 用途 |
|----|------|
| Apache POI 5.3.0 | .docx / .xlsx 解析 |
| Apache PDFBox 3.0.3 | .pdf 文本提取 |
| DashScope OSSUtils | 二进制文件上传 OSS |
