...
public class ChatService {
    ...
    public ChatService(
            ...
            @Value("${llm.max-history:20}") int maxHistory) {
    ...
    public String chat(String userId, String userMessage) {
        ...
        if (history.size() > maxHistory) {
            List<Map<String, String>> trimmed = new ArrayList<>(history.subList(history.size() - maxHistory, history.size()));
            conversations.put(userId, trimmed);
            history = trimmed;
        }

        String reply = llmService.chat(history);

        history.add(Map.of("role", "assistant", "content", reply));

        log.info("AI 对话: userId={}, userMsg={}, reply={}", userId, userMessage, reply.substring(0, Math.min(50, reply.length())));
        return reply;
    }
